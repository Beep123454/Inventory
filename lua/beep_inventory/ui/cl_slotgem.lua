...
function PANEL:GetItemInfo(data)
    return self.Item 
end
function PANEL:SetItemInfo(data)
    ...
    self.exit = BUi.Create("DButton",self.infoholder)
    self.exit:Stick(RIGHT)
    self.exit:DockMargin(10, 0, 0, 60)
    self.exit:SetWide(40)
    self.exit:SetText("")
    self.exit:BUi():ClearPaint():Background(Color(56, 56, 56,200), 5):FadeIn(0.5):On("Paint", function(s, w, h)
        draw.RoundedBox(5, 1, 1, w - 2, h - 2,  BCORE.F4.colors.accent)
        BUi.DrawImgur(0,0,w,h, "0cJXWbc",color_white)
    ...
    if  BCORE.Inventory.config.Rarities[self.Item.rarity].weight > 4  then
        for i = 1, BCORE.Inventory.config.Rarities[self.Item.rarity].sockets do
            ...
            socket:BUi():ClearPaint():On("Paint", function(s, width, height)
                ...
                //draw.SimpleText(i, "BCORE.Inventorys.20", width/2, height/2, color_white, TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
            end)
                ...
                if dropped then
                    if receiver:GetChildren()[1] then
                       return
                            ...
                            PrintTable( droppedPanels[1]:GetItem())
        ...
        if  not table.IsEmpty(self.Item.customData) then
            ...
            for k, v in SortedPairs(self.Item.customData) do
                ...
                if self.Item.itemType == "Modifier" then 
                    draw.SimpleText(k .. ": " .. val .. " ", "BCORE.Inventorys.25", self.navbar:GetWide() * .37, pos, color_white, TEXT_ALIGN_LEFT)
                    draw.SimpleText("+" .. val, "BCORE.Inventorys.25", self.navbar:GetWide() * .37 + offsetx, pos, Color(28,208,28), TEXT_ALIGN_LEFT)
                else
                    ...
                    draw.SimpleText(k .. ": " .. val .. " ", "BCORE.Inventorys.25", self.navbar:GetWide() * .37, pos, color_white, TEXT_ALIGN_LEFT)
                    draw.SimpleText(substat, "BCORE.Inventorys.25", self.navbar:GetWide() * .37 + offsetx, pos, redorgreen, TEXT_ALIGN_LEFT)
...
vgui.Register("[BCORE][UI][SLOT_GEM]", PANEL, "DPanel")
